$(document).ready(function () {
	$("#riwayat-dinas-luar-pegawai-table").DataTable({
		paging: true,
		lengthChange: false,
		searching: true,
		ordering: true,
	...
	$("#riwayat-dinas-luar-pegawai-table").on("DOMSubtreeModified", function () {
		reloadEvent();
	});
	...
	function reloadEvent() {
					...
					try {
						...
						if (cuti["foto"] != "") {
							$("#modal-detail #user-photo").attr(
								"src",
								"data:image/jpeg;base64," + cuti["foto"]
							);
						} else {
							$("#modal-detail #user-photo").attr(
								"src",
						...
						$("#modal-detail #tanggal-cuti-mulai").html(
							moment(cuti["tanggal_cuti_mulai"]).format("DD/MM/YYYY")
						);
						$("#modal-detail #tanggal-cuti-akhir").html(
							moment(cuti["tanggal_cuti_akhir"]).format("DD/MM/YYYY")
						);
		...
		$(".hapus-btn").click(function () {
			...
			$("#modal-hapus .modal-body p").html(
				"Anda yakin menghapus dinas luar ini ?"
			);
